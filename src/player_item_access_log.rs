...
/// Mirrors the `AVPlayer` framework counterpart for `PlayerItemAccessLog`.
#[derive(Debug)]
...
retain_release_wrapper!(
    PlayerItemAccessLog,
    release = ffi::av_player_item_access_log_release
);
...
impl PlayerItemAccessLog {
    fn info(&self) -> Result<AccessLogPayload, AVPlayerError> {
        ...
        let json_ptr = unsafe { ffi::av_player_item_access_log_info_json(self.ptr, &raw mut err) };
        if json_ptr.is_null() {
